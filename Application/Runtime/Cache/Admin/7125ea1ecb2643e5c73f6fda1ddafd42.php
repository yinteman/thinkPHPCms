		...
		<div class="row">
			<div class="col-lg-12">
				...
				<ol class="breadcrumb">
					...
					<li class="active">
						<i class="fa fa-edit"></i> 修改推荐推荐位内容
					</li>
				</ol>
			</div>
		</div>
			...
			<div class="col-lg-6">
				...
				<form class="form-horizontal" id="singcms-form">
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<select class="form-control" name="position_id">
									...
									<?php if(is_array($positions)): foreach($positions as $key=>$pos): ?><option value="<?php echo ($pos["id"]); ?>"
										<?php if($pos['id'] == $posID): ?>selected="selected"<?php endif; ?>> <?php echo ($pos["name"]); ?></option><?php endforeach; endif; ?>
							</select>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" name="news_id" class="form-control" id="inputname" value="<?php echo ($rows["news_id"]); ?>">
						</div>
					</div>
					<div class="form-group">
					<label for="inputPassword3" class="col-sm-2 control-label">状态:</label>
					<div class="col-sm-5">
						<input type="radio" name="status" id="optionsRadiosInline1" value="1" <?php if($rows['status'] == 1): ?>checked<?php endif; ?>> 开启
						<input type="radio" name="status" id="optionsRadiosInline2" value="0"
						<?php if($rows['status'] == 0): ?>checked<?php endif; ?>> 关闭
					</div>
...
<script>
	...
	/***编写js代码*/
  var isthumb='<?php echo ($rows["thumb"]); ?>';
  if (isthumb){
