    ...
    <div class="container-fluid" >
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <ol class="breadcrumb">
            <li>
              <i class="fa fa-dashboard"></i>  <a href="<?php echo U('Content');?>">文章管理</a>
            </li>
            <li class="active">
          ...
          <div class="col-md-3">
            <div class="input-group">
              ...
              <select class="form-control" name="catid">
                ...
                <?php if(is_array($wibeName)): foreach($wibeName as $key=>$name): ?><option value="<?php echo ($name["menu_id"]); ?>" ><?php echo ($name["name"]); ?></option><?php endforeach; endif; ?>
              </select>
            </div>
          </div>
              ...
              <table class="table table-bordered table-hover singcms-table">
                ...
                <tbody>
                 <?php if(is_array($rows)): foreach($rows as $key=>$row): ?><tr>
                   <td>
                    <input type="checkbox" name="pushcheck" value="<?php echo ($row["news_id"]); ?>" /></td> 
                    <td><input size=4 type='text'  name='<?php echo ($row["news_id"]); ?>' value="<?php echo ($row["listorder"]); ?>"/></td><!--6.7-->
                    <td><?php echo ($row["news_id"]); ?></td>
                    <td><?php echo ($row["title"]); ?></td>
                    <td><?php echo (getCatName($row["catid"])); ?></td>
                    <td><?php echo (getCopyName($row["copyfrom"])); ?></td>
                    <td>
                      <?php echo (isThumb($row["thumb"])); ?>
                    </td>
                    <td><?php echo (date("y-m-d",$row["create_time"])); ?></td>
                    <td><span  attr-status="<?php echo ($row["status"]); ?>"  attr-id="<?php echo ($row["news_id"]); ?>" class="sing_cursor singcms-on-off" id="singcms-on-off" >
                    <?php echo (getStatus($row["status"])); ?></span></td>
                    <td><span class="sing_cursor glyphicon glyphicon-edit" aria-hidden="true" id="singcms-edit" attr-id="" 
                    onclick="edit(<?php echo ($row['news_id']); ?>)"></span>
                      <a href="javascript:void(0)" id="singcms-delete"  attr-id=""  attr-message="删除"  onclick="deleteMenu(<?php echo ($row['news_id']); ?>)">
